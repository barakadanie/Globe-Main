...
class ConfirmedPayments extends StatelessWidget {
  @override
  ...
  Widget _buildBody() {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('Mysales').where('Confirmation', isEqualTo: true).snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Text('No confirmed payments available');
        } else {
          List<DataColumn> columns = [
            ...
            DataColumn(label: Text('Confirmation', style: TextStyle(color: Colors.blue))),
          ];
